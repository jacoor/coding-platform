...
def read_file(file_path):
    ...
    with open(file_path, "r") as file:
        content = file.read()
    logging.debug(
        f"File content from {file_path}: {content[:100]}..."
...
"""
    logging.debug(
        f"Prepared script: {full_script[:100]}..."
    )  # Log first 100 characters
    return full_script
...
def submit_code(code):
    ...
    try:
        ...
        if response.status_code == 201:
            ...
            logging.info(
                f"Code submission successful. Token received: {submission['token']}"
            )
            return submission["token"]
        else:
            logging.error(
                f"Error during submission: {response.status_code}, {response.text}"
            )
            return None
...
def get_submission_result(token):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logging.error(
                f"Error fetching result: {response.status_code}, {response.text}"
            )
            return None
    except requests.exceptions.RequestException as e:
